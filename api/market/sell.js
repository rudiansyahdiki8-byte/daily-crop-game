  ...
  try {
    ...
    const result = await userRef.firestore.runTransaction(async (t) => {
      ...
      if (itemName) {
         ...
         // Jual Semua (Sell All) - Kecuali Consumables
      ...
      // 5. LOGIC AFFILIATE (UPDATE BARU)
      // Mencatat detail bonus ke teman spesifik
      if (userData.uplineId) {
          const uplineRef = getUserRef(userData.uplineId);
          const uplineDoc = await t.get(uplineRef);
          
          if (uplineDoc.exists) {
              // Ambil persen dari Config (Default 10%)
              ...
              if (commission > 0) {
                  ...
                  // Ambil data statistik teman ini di database upline
                  ...
                  t.update(uplineRef, {
                      ...
                      // Update spesifik user ini
                      [`referrals.${userId}`]: {
                          ...myStats,
                          username: userData.username || myStats.username, // Update nama jika ganti
