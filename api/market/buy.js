...
export default async function handler(req, res) {
  ...
  try {
    ...
    await userRef.firestore.runTransaction(async (t) => {
      ...
      let cost = 0;
      let type = ''; // 'SLOT', 'STORAGE', 'BUFF'
      let value = null;

      // 1. CEK ITEM APA YANG DIBELI
      if (itemId === 'SLOT_2') {
        cost = LAND_PLOTS.SLOT_2.cost;
        type = 'SLOT'; value = 2;
      } else if (itemId === 'SLOT_3') {
        cost = LAND_PLOTS.SLOT_3.cost;
        type = 'SLOT'; value = 3;
      } else if (itemId === 'STORAGE_20') {
        cost = 2000;
        type = 'STORAGE'; value = 20;
      } 
      // LOGIC BARU UNTUK CONSUMABLES 
      else if (CONSUMABLES[itemId]) {
        cost = CONSUMABLES[itemId].price;
        type = 'BUFF';
        value = itemId; // Key buff-nya
      } else {
      ...
      if (type === 'SLOT') {
        ...
        updateData.storageLimit = (userData.storageLimit || 50) + value;
      } 
      else if (type === 'BUFF') {
        // Buff berlaku 24 jam dari SEKARANG
        const expireTime = Date.now() + (24 * 60 * 60 * 1000); 
        // Simpan di object 'buffs'
        // Contoh: buffs.SPEED_SOIL = 173000000000
        updateData[`buffs.${value}`] = expireTime;
      }
