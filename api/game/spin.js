...
// [PENTING] IMPORT DARI CONFIG UTAMA
// Agar hadiah dan harga sinkron dengan Frontend
import { SPIN_PRIZES, SPIN_CONFIG, REFERRAL_BONUSES } from '../../src/config/gameConstants.js';

// Logic Random (Menggunakan Bobot/Weight)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await userRef.firestore.runTransaction(async (t) => {
      ...
      // --- HELPER: LOGIC BONUS REFERRAL (JACKPOT) ---
      // Fungsi kecil untuk memberi bonus ke upline jika menang Jackpot
      ...
      if (mode === 'FREE') {
        ...
        // TAHAP A: ROLL (Muter doang, Server tentukan hadiah)
        if (step === 'ROLL') {
            ...
            // [FITUR BARU] Cek Jackpot Bonus
            if (reward.id === 'JACKPOT') {
                await giveJackpotBonusToUpline(reward.val);
            }

            // Simpan PENDING REWARD (Belum masuk saldo user)
            t.update(userRef, { 
                lastFreeSpin: now,
        ...
        // TAHAP B: CLAIM (Setelah nonton iklan)
        if (step === 'CLAIM') {
      ...
      // ==========================================
      // 2. MODE PAID SPIN (Langsung Dapat)
      // ==========================================
      else if (mode === 'PAID') {
        ...
        // [FITUR BARU] Cek Jackpot Bonus (Paid juga dapat bonus upline)
        if (reward.id === 'JACKPOT') {
            await giveJackpotBonusToUpline(reward.val);
        }
