...
async upgradePlan(planKey, priceUsdt) {
    UIEngine.showRewardPopup("CONFIRM UPGRADE", `Upgrade to ${planKey} for ${priceUsdt} USDT?`, async () => {
        UIEngine.showRewardPopup("PROCESSING", "Verifying membership status...", null, "...");

        try {
            const response = await fetch('/api/shop/upgrade', {
                method: 'POST',
            ...
            if (result.success) {
                await GameState.load(); // Reload data plan terbaru
                UIEngine.updateHeader();
                this.init(); // Re-render halaman plan
                UIEngine.showRewardPopup("SUCCESS", `Welcome to ${planKey} tier!`, null, "AWESOME");
            } else {
