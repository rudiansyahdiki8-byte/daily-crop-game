const FarmSystem = {
    ...
    dailyTasks: [
        ...
        { id: 'claim_spin', name: 'Hack Lucky Slot', icon: 'fa-dharmachakra', reward: 0, action: 'spin' },
    ...
    init() {
        if(!GameState.farmPlots || GameState.farmPlots.length === 0) {
            GameState.farmPlots = Array(12).fill(null).map((_, i) => ({
                id: i + 1, status: 'locked', plant: null, timer: 0
            }));
        }

        const now = Date.now();
        const lastActive = GameState.user.lastActive || now;
        const secondsPassed = Math.floor((now - lastActive) / 1000);

        if (secondsPassed > 0) {
            GameState.farmPlots.forEach(plot => {
                if (plot.status === 'growing') {
                    plot.timer -= secondsPassed; 
                    if (plot.timer <= 0) {
                        plot.status = 'ready';
                        plot.timer = 0;
                    }
    ...
    renderLayout() {
        ...
        header.innerHTML = `
            <div class="flex items-center gap-3 w-1/3">
                <div class="w-10 h-10 bg-cyan-500/10 rounded-xl flex items-center justify-center border border-cyan-500/30">
                     <i class="fas fa-microchip text-cyan-400 text-lg animate-pulse"></i>
                </div>
                <div>
                    <h3 class="text-[9px] font-black text-white uppercase tracking-wider leading-tight">
                        ${GameState.user.username || 'NETRUNNER'}
                    </h3>
                    <p class="text-[7px] text-cyan-500 font-bold uppercase italic">Neural Mainframe</p>
                </div>
            </div>
            ...
            <div id="target-warehouse-icon" onclick="WarehouseSystem.show()" class="w-1/3 flex justify-center cursor-pointer active:scale-90 transition-transform">
                <div class="relative w-12 h-12 bg-black/60 rounded-2xl border-2 border-[#ff0055] shadow-[0_0_10px_#ff0055] flex items-center justify-center">
                    <i class="fas fa-database text-white text-xl drop-shadow-md"></i>
                    <div class="absolute -top-2 -right-2 bg-[#ff0055] text-white text-[7px] font-black px-1.5 rounded-full border border-white uppercase">Vault</div>
                </div>
            </div>

            <div class="w-1/3 flex justify-end">
                <button onclick="SpinSystem.show()" class="group flex items-center gap-2 bg-[#ff0055]/10 border border-[#ff0055]/50 px-3 py-1.5 rounded-xl hover:bg-[#ff0055]/20 transition-all active:scale-95">
                    <div class="text-right">
                        <p class="text-[6px] text-[#ff0055] font-bold uppercase italic">Neural</p>
                        <p class="text-[8px] font-black text-white uppercase">Hack</p>
                    </div>
                    <i class="fas fa-dharmachakra text-[#ff0055] animate-spin-slow group-hover:text-white"></i>
                </button>
            </div>
        `;
    ...
    plantAll() {
        ...
        GameState.farmPlots.forEach(plot => {
            if (plot.status === 'empty') {
                ...
                const baseTime = this.plantData[plot.plant] ? this.plantData[plot.plant].time : 5;
                plot.timer = Math.ceil(baseTime * speedMultiplier);
                
                planted++;
            }
        });
        
        if (planted > 0) this.renderFarmGrid();
    },
    ...
    renderFarmGrid() {
        ...
        for (let i = 0; i < limit; i++) {
            ...
            if (plot.status === 'disabled') {
                ...
                content = `<div class="h-full w-full bg-black/80 border border-cyan-500/30 flex flex-col items-center justify-center relative"><div class="absolute inset-0 bg-cyan-500/5 animate-pulse"></div><i class="fas fa-satellite text-cyan-400 text-3xl animate-flicker mb-2"></i><span class="text-[8px] font-mono font-bold text-cyan-300 tracking-widest bg-black/60 px-2 py-0.5">${this.formatTime(plot.timer)}</span><div class="absolute bottom-0 left-0 h-0.5 bg-cyan-400" style="width: ${100 - (plot.timer / (this.plantData[plot.plant]?.time || 10) * 100)}%"></div></div>`;
            } else if (plot.status === 'ready') {
                const img = this.plantData[plot.plant] ? this.plantData[plot.plant].img : 'https://img.icons8.com/nolan/96/data-configuration.png';
                content = `<div class="h-full w-full bg-black border border-[#ff0055]/50 flex flex-col items-center justify-center relative shadow-[0_0_15px_rgba(255,0,85,0.2)] group cursor-pointer"><div class="absolute inset-0 bg-[#ff0055]/10 animate-pulse"></div><img src="${img}" class="w-16 h-16 object-contain group-hover:scale-110 transition-transform"><div class="absolute bottom-2 bg-[#ff0055] text-white text-[6px] font-black px-3 py-0.5 uppercase italic">Extract</div></div>`;
