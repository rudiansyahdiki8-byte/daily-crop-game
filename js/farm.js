...
renderLayout() {
        ...
        // ... (Pastikan isi header sama seperti sebelumnya) ...
        header.innerHTML = `
            ...
            <div onclick="SpinSystem.show()" class="spin-orb ${((Date.now() - (GameState.user.spin_free_cooldown || 0)) > window.GameConfig.Spin.CooldownFree) ? "spin-ready" : "bg-gray-800 border-gray-600 grayscale opacity-80"} active:scale-90 cursor-pointer">
        ...
        // 3. FLOATING MENU (UPDATED ICON)
        const fabContainer = document.createElement('div');
        fabContainer.className = "fab-menu";
        
        // Perhatikan bagian <div id="fab-icon-container"> di bawah ini:
        fabContainer.innerHTML = `
            <button onclick="FarmSystem.toggleTaskMenu()" class="fab-btn bg-white/90 backdrop-blur border border-emerald-400 relative z-50">
            ...
            <div id="task-list-content" class="fab-content w-48 flex flex-col gap-2"></div>
        `;
    ...
    renderTaskButtons() {
        ...
        plantBtn.className = "w-full bg-emerald-500/20 hover:bg-emerald-500/40 border border-emerald-500/50 rounded-xl py-1 px-2 flex items-center gap-3 transition-all active:scale-95";        plantBtn.innerHTML = `<div class="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white"><i class="fas fa-seedling text-xs"></i></div><span class="text-[9px] font-black uppercase text-white">Plant Random</span>`;
...
// Kode Baru (Lebih Pendek)
    ...
    handleTaskClick(task, btnElement) {
        ...
        AdsManager.showAd(`Task: ${task.name}`, async () => {
            // --- LOGIKA HADIAH (Dijalankan setelah iklan selesai) ---
