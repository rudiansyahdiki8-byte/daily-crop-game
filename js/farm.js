...
const FarmSystem = {
    ...
    // Konfigurasi Visual Slot (Total 4 Slot yang terlihat sesuai dokumen)
    maxVisibleSlots: 4,

dailyTasks: [
        // Tugas Ringan (Reward 120 PTS)
        { id: 'daily_login', name: 'Daily Login', icon: 'fa-calendar-check', reward: 120 },
        { id: 'visit_farm', name: 'Visit Farm House', icon: 'fa-walking', reward: 120 },
        { id: 'free_reward', name: 'Free Gift', icon: 'fa-gift', reward: 120 },
        { id: 'clean_farm', name: 'Clean Farm Area', icon: 'fa-broom', reward: 120 },

        // Tugas Menengah (Reward 150 PTS)
        { id: 'water_plants', name: 'Water Plants', icon: 'fa-tint', reward: 150 },
        { id: 'fertilizer', name: 'Add Fertilizer', icon: 'fa-seedling', reward: 150 },
        { id: 'kill_pests', name: 'Pest Control', icon: 'fa-bug', reward: 150 },

        // Tugas Utama (Reward 180 PTS - High Value)
        { id: 'harvest_once', name: 'Harvest Crop', icon: 'fa-sickle', reward: 180 },
        { id: 'sell_item', name: 'Sell at Market', icon: 'fa-coins', reward: 180 },
    ...
    updateSlotStatus() {
        ...
        GameState.farmPlots.forEach((plot, index) => {
            ...
            // LOGIKA PENGUNCIAN SLOT (481-485)
    ...
    async plantAll() {
        ...
        this.updateSlotStatus(); // Pastikan status terbaru
        ...
        emptyPlots.forEach(plot => {
            plot.status = 'growing';
            plot.plant = DropEngine.roll(); // RNG dari state.js (sudah terhubung ke Herbs Baru)
            ...
            GameState.farmPlots.forEach(plot => {
                if (plot.status === 'growing') {
                    if (now >= plot.harvestAt) {
                        ...
                        // Update visual timer jika perlu (di sini kita update tiap detik lewat renderFarmGrid jika mau, tapi boros)
                        // Kita hanya render ulang jika status berubah atau via timer internal renderFarmGrid yang lebih ringan
                        change = true; 
                    }
                }
            });
    ...
    renderFarmGrid() {
        ...
        // Panggil update status untuk memastikan slot lock/unlock sesuai data user terbaru
        ...
        for (let i = 0; i < displayLimit; i++) {
            ...
            if (plot.status === 'disabled') {
                // Slot Masa Depan (Locked Permanen di versi ini)
                ...
                // Slot Bisa Dibeli
                let priceLabel = i === 1 ? "5K PTS" : "500K PTS"; // Harga Slot 2 & 3
                content = `
