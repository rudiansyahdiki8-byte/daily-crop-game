...
const SpinSystem = {
    ...
    cooldownTime: 1 * 60 * 60 * 1000, 
    isSpinning: false,

    // DEFINISI 8 SEGMEN RODA (Visual Urut Jarum Jam)
    segments: [
        { id: 'coin_50', type: 'coin', val: 50, label: '50 PTS', color: '#60a5fa', icon: 'fa-coins' },       
        { id: 'herb_common', type: 'herb', rarity: 'Common', label: 'Common', color: '#4ade80', icon: 'fa-leaf' }, 
        { id: 'coin_200', type: 'coin', val: 200, label: '200 PTS', color: '#fbbf24', icon: 'fa-coins' },     
        { id: 'herb_common', type: 'herb', rarity: 'Common', label: 'Common', color: '#4ade80', icon: 'fa-leaf' }, 
        { id: 'coin_100', type: 'coin', val: 100, label: '100 PTS', color: '#60a5fa', icon: 'fa-coins' },     
        { id: 'herb_rare', type: 'herb', rarity: 'Rare', label: 'Rare', color: '#c084fc', icon: 'fa-star' },       
        { id: 'jackpot', type: 'jackpot', val: 1000, label: 'JACKPOT', color: '#f43f5e', icon: 'fa-gem' },    
        { id: 'coin_50', type: 'coin', val: 50, label: '50 PTS', color: '#60a5fa', icon: 'fa-coins' }         
    ],
            ...
            <div class="glass w-full max-w-sm rounded-[2.5rem] p-6 border border-white/10 text-center relative overflow-hidden shadow-2xl flex flex-col items-center">
                ...
                <div class="w-full flex flex-col gap-3 relative z-20">
                    ...
                    <button id="btn-spin-paid" onclick="SpinSystem.start('paid')" class="w-full bg-white/5 hover:bg-white/10 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-white/5 active:scale-95 transition-all">
                        <span class="flex items-center justify-center gap-2"><i class="fas fa-coins text-amber-400"></i> Spin (150 PTS)</span>
                    </button>
                </div>
            </div>
        `;
    ...
    async start(type) {
        ...
        // 1. CEK SYARAT & PAYMENT DULU
        if (type === 'paid') {
            if (GameState.user.coins < 150) {
                UIEngine.showRewardPopup("NO COINS", "Need 150 PTS to spin.", null, "CLOSE");
                return;
            }
            GameState.user.coins -= 150;
        } else if (type === 'free') {
            // Anti-Cheat: Langsung kunci cooldown SEBELUM muter
            // Kalau user refresh saat liat hadiah jelek, spin tetap hangus.
            GameState.user.spin_free_cooldown = Date.now();
        }

        // Simpan state pembayaran/cooldown agar aman
    ...
    executeSpin(type) {
        ...
        // 40% Zonk/Koin Kecil, 30% Common, 20% Koin Sedang, 9% Rare, 1% Jackpot
    ...
    handleResult(prize, type) {
        ...
        // --- LOGIC FREE VS PAID (ALUR BEDA DISINI) ---
        ...
        if (type === 'paid') {
            ...
            // Kita pakai Popup khusus agar user tidak bisa close tanpa nonton (atau close = hangus)
            UIEngine.showRewardPopup(
                ...
                () => {
                    // Action Tombol Claim
                    UIEngine.showRewardPopup("ADVERTISEMENT", "Loading Ad... 3s", null, "...");
                    setTimeout(() => {
                        this.grantReward(prize, finalPrizeKey);
                        UIEngine.showRewardPopup("CLAIMED", `${finalPrizeName} added to inventory!`, null, "NICE");
                    }, 3000);
                }, 
                "WATCH AD & CLAIM" // Teks Tombol
            );
        }
...
// Jangan lupa tambahkan method updateTimer dan close() standar agar tidak error
