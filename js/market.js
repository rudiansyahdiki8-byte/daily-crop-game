// js/market.js
// ==========================================
// MARKET SYSTEM (UPDATED)
// Fitur: Sell Bonus Membership & Dynamic Shop Limits
// ==========================================
...
const MarketSystem = {
    ...
    shopItems: [
        // --- ASSETS (PERMANENT) ---
        { 
            id: 'land_slot', type: 'land', 
            name: "Extra Land Plot", 
            icon: "fa-map-location-dot", color: "text-emerald-400", 
            price: window.GameConfig.ShopItems.LandPrice || 10000, 
            desc: "Expand production. Limit depends on Membership.",
            category: "Asset"
        },
        { 
        ...
        { 
            ...
            desc: "+15% Market Value (24h Active)",
            category: "Consumable"
        },
        { 
    ...
    async triggerAffiliateCommission(totalSales) {
        ...
        if (GameState.user.upline && commission > 0) {
            ...
            // Note: Logika kirim saldo ke upline ada di Cloud Function/Backend
            // Di sisi client ini hanya trigger visual/log
        }
    },
    ...
    getSellMultiplier() {
        ...
        // 3. MEMBERSHIP BONUS (+5% / +10% / +50%) - [UPDATED]
        const userPlan = (GameState.user && GameState.user.plan) ? GameState.user.plan : 'FREE';
        const planConfig = window.GameConfig.Plans[userPlan];
        
        if (planConfig && planConfig.sellBonus > 0) {
            multiplier += planConfig.sellBonus;
        }
    ...
    async processSell(key, qty, price) {
        ...
        UIEngine.showRewardPopup("TRANSACTION COMPLETE", `Sold ${qty} units. Revenue: ${totalEarn} PTS.`, null, "CONFIRM");
    },
    ...
    renderSellInventory() {
        ...
        // Tampilkan Bonus Aktif
        const multiplier = this.getSellMultiplier();
        const bonusPercent = Math.round((multiplier - 1) * 100);
        if (bonusPercent > 0) {
            container.innerHTML += `<div class="col-span-2 mb-2 bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-2 text-center text-[9px] text-emerald-400 font-bold uppercase"><i class="fas fa-arrow-up"></i> Active Sell Bonus: +${bonusPercent}%</div>`;
        }
    ...
    // --- RENDER SHOP (UPDATED: DYNAMIC LIMIT) ---
    renderShopItems() {
       ...
       this.shopItems.forEach(item => {
            ...
            // 1. Logic Item "Land" (Buyable Slots)
            if (item.type === 'land') {
                const purchased = GameState.user.landPurchasedCount || 0;
                const limit = planConfig.buyableSlots; // Ambil dari Config Plan
                
                if (purchased >= limit) {
                    disabled = true;
                    btnText = "MAX LIMIT";
                    btnClass = "bg-gray-800 text-gray-500 border-transparent cursor-not-allowed";
                }
            } 
            // 2. Logic Item "Buff"
            ...
            // 3. Logic Item "Storage"
            else if (item.type === 'storage') {
                 // Jika Unlimited, disable tombol beli storage
                 if (planConfig.storage === Infinity) {
                     disabled = true;
                     btnText = "UNLIMITED";
                     btnClass = "bg-purple-900/50 text-purple-400 border-purple-500/30";
                 } else {
