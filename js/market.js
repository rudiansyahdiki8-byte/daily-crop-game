...
const MarketSystem = {
    ...
    async processSell(key, qty, price) {
        ...
        // Save history (Logic Updated untuk cegah error)
    ...
    renderSellInventory() {
        ...
        Object.keys(GameState.warehouse).forEach(key => {
            ...
            if (count > 0) {
                isEmpty = false;
                // Menggunakan HerbData yang sudah terkoneksi config (dari langkah sebelumnya)
    ...
    getSellMultiplier() {
        ...
        if (activeBuffs['sell_bonus'] && activeBuffs['sell_bonus'] > Date.now()) multiplier += 0.2;
        return multiplier;
    },
