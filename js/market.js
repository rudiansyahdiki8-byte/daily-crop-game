...
const MarketSystem = {
    ...
    // DATA ITEM SHOP (Sesuai Dokumen Ekonomi)
    shopItems: [
        // --- PERMANENT UPGRADES ---
        { 
    ...
    async triggerAffiliateCommission(totalSales) {
        ...
        // 2. Logic Komisi Upline (Hanya log console dulu, logic backend di server nanti)
    ...
    renderLayout(container) {
        container.innerHTML = '';
        container.className = "h-full flex flex-col p-5 animate-in pb-28"; // Pb-28 agar tidak tertutup nav
    ...
    async processSell(key, qty, price) {
        ...
        this.renderSellInventory();
        this.calculatePreview();
        UIEngine.showRewardPopup("SOLD", `Sold ${qty}x for ${totalEarn} PTS!`, null, "OK");
    },
    ...
    renderSellInventory() {
        ...
        Object.keys(GameState.warehouse).forEach(key => {
            ...
            if (count > 0) {
                ...
                // Card Inventory yang lebih bersih
                container.innerHTML += `
                    <div onclick="MarketSystem.openSellModal('${key}', ${count}, ${currentPrice})" class="glass p-3 rounded-2xl flex items-center justify-between border border-white/5 group active:scale-95 transition-all cursor-pointer hover:bg-white/5 hover:border-emerald-500/30">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 bg-black/20 rounded-xl flex items-center justify-center border border-white/5">
                                <img src="${plantInfo.img}" class="w-8 h-8 object-contain drop-shadow-md group-hover:scale-110 transition-transform">
                            </div>
                            <div>
                                <h4 class="text-[9px] font-black text-white uppercase tracking-wide">${plantInfo.name}</h4>
                                <span class="text-[8px] text-gray-400">Stock: <b class="text-white">${count}</b></span>
                            </div>
                        </div>
                        <div class="flex flex-col items-end">
                            <span class="text-[10px] font-black text-emerald-400 flex items-center gap-1">${currentPrice} <span class="text-[6px]">PTS</span></span>
                            <span class="text-[7px] text-gray-500">TAP TO SELL</span>
                        </div>
                    </div>`;
            }
        });
    ...
    getSellMultiplier() {
        let multiplier = 1;
        if (GameState.user.adBoosterCooldown > Date.now()) multiplier += 0.2; // +20%
        
        // Cek Buff dari Item Market
        const activeBuffs = GameState.user.activeBuffs || {};
        if (activeBuffs['sell_bonus'] && activeBuffs['sell_bonus'] > Date.now()) multiplier += 0.2; // +20%
    ...
    renderShopItems() {
       ...
       this.shopItems.forEach(item => {
            ...
            if (item.category === 'Asset') {
                // Layout Horizontal untuk Asset agar terlihat Mahal
                const assetHtml = `
    ...
    async buyItem(id, price) {
        ...
        UIEngine.showRewardPopup("CONFIRM BUY", `Purchase ${item.name} for ${price.toLocaleString()} PTS?`, async () => {
            ...
            if (item.type === 'land') {
                ...
                // Update status di FarmSystem nanti dilakukan otomatis saat init/refresh
            } 
            else if (item.type === 'storage') {
                ...
                GameState.user.activeBuffs[item.buffKey] = Date.now() + 86400000; // 24 Jam
            }
    ...
    applyPriceBooster() {
        UIEngine.showRewardPopup("PRICE BOOSTER", "Watch Ad for +20% sell price?", async () => {
            // Simulasi Iklan
            UIEngine.showRewardPopup("ADVERTISEMENT", "Watching Ad... 5s", null, "...");
            setTimeout(async () => {
                GameState.user.adBoosterCooldown = Date.now() + 86400000; // 24 Jam
                await GameState.save();
                this.checkBoosterStatus();
                this.calculatePreview();
