...
async process() {
    // 1. Ambil elemen input dari UI [cite: 326]
    const inputAmt = document.getElementById('wd-amount');
    const inputAddr = document.getElementById('wd-address');
    
    if (!inputAmt || !inputAddr) return; 

    // 2. Sanitasi dan Validasi Input [cite: 327]
    const amountPTS = parseInt(inputAmt.value);
    const rawAddress = inputAddr.value;
    const address = rawAddress.trim().toLowerCase().replace(/\s/g, ''); 
    
    // Perbarui tampilan input jika ada spasi yang dihapus [cite: 327]
    if (rawAddress !== address) inputAddr.value = address;

    // 3. Validasi Saldo & Limit di Sisi Klien (Pengecekan Awal) [cite: 328, 330]
    const cfg = window.GameConfig.Finance; 
    const minLimit = GameState.user.has_withdrawn ? cfg.MinWdOld : cfg.MinWdNew; 

    if (!amountPTS || amountPTS < minLimit) {
        UIEngine.showRewardPopup("VALIDATION ERROR", `Minimum required: ${minLimit.toLocaleString()} PTS`, null, "FIX"); 
        return;
    }

    if (amountPTS > GameState.user.coins) {
        UIEngine.showRewardPopup("INSUFFICIENT FUNDS", "Balance too low for this transaction.", null, "CLOSE"); 
        return;
    }

    if (!address) {
        UIEngine.showRewardPopup("MISSING DATA", "Please provide a destination address.", null, "FIX"); 
        return;
    }

    // 4. Konfirmasi User Sebelum Kirim ke Server [cite: 334]
    const rate = this.rates[this.selectedCurrency] || 0; 
    const estimatedCrypto = (amountPTS * rate * this.SAFETY_MARGIN).toFixed(8); 

    UIEngine.showRewardPopup("CONFIRM TRANSFER", 
        `Authorize withdrawal of **${amountPTS.toLocaleString()} PTS** (~${estimatedCrypto} ${this.selectedCurrency}) to ${address}?`, 
        async () => {
            
            // Tampilkan Indikator Loading [cite: 334]
            UIEngine.showRewardPopup("PROCESSING", "Contacting Financial Gateway...", null, "WAITING...");

            try {
                // 5. Eksekusi Panggilan API ke Backend [cite: 335]
                const response = await fetch('/api/withdraw', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        initData: window.Telegram.WebApp.initData, // Keamanan Auth Telegram 
                ...
                if (result.success) {
                    // 6. Sinkronisasi Data Setelah Berhasil [cite: 338, 341]
                    // Kita muat ulang data dari server untuk memastikan saldo akurat [cite: 615]
                    await GameState.load();
                    
                    // Update tampilan UI [cite: 341, 642]
                    UIEngine.updateHeader(); 
                    UIEngine.closeWithdraw(); 
                    
                    UIEngine.showRewardPopup("SUCCESSFUL", 
                        "Transaction processed! Funds have been sent to your wallet.", 
                        null, "EXCELLENT"); 
                } else {
                    // Tangani Error dari Server (Misal: Limit tidak tercapai atau saldo tidak cukup di server) [cite: 343]
                    UIEngine.showRewardPopup("GATEWAY ERROR", result.message || "Transaction denied by server.", null, "RETRY"); 
                }

            } catch (error) {
                // Tangani Masalah Jaringan [cite: 344]
                console.error("Network Error:", error);
                UIEngine.showRewardPopup("NETWORK ERROR", "Unable to reach server. Check your connection.", null, "CLOSE"); 
            }
        }, 
    "AUTHORIZE");
    },
