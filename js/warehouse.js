...
const WarehouseSystem = {
    ...
    getMaxLimit() {
        // AMBIL DATA DARI MEMBERSHIP AKTIF
        const activePlan = GameState.getActivePlan(); // Fungsi baru di state.js
        const extra = GameState.user.extraStorage || 0; 
        
        // Logika Landlord (Unlimited)
        // Di Config.js kita set Infinity, tapi untuk coding aman kita return angka fantastis
        if (activePlan.storage === Infinity) {
            return 999999999; 
        }

        // Rumus: Kapasitas Plan + Beli Tambahan
        return activePlan.storage + extra;
    },
    ...
    isFull(amountToAdd = 1) {
        ...
        // Jika max sangat besar (Unlimited), tidak akan pernah full
        if (max >= 999999999) return false;
    ...
    renderLayout() {
        ...
        // Handling Tampilan Unlimited (Landlord)
        const isUnlimited = max >= 999999999;
        const maxLabel = isUnlimited ? "∞" : max;
            ...
            <div class="mb-6 relative z-10">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-12 h-12 bg-gradient-to-br from-amber-700 to-amber-900 rounded-2xl flex items-center justify-center border border-white/10 shadow-lg relative">
                        ...
                        ${(!isUnlimited && percent >= 100) ? '<div class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-bounce"></div>' : ''}
                    </div>
                    <div>
                ...
                <div class="w-full h-8 bg-black/60 rounded-xl border border-white/10 relative overflow-hidden p-1">
                    <div id="wh-progress-bar" class="h-full rounded-lg ${barColor} transition-all duration-700 ease-out relative overflow-hidden" style="width: ${isUnlimited ? 100 : percent}%">
                        <div class="absolute inset-0 bg-white/20 animate-[shimmer_2s_infinite]"></div>
                    </div>
                    <div class="absolute inset-0 flex items-center justify-center"><span class="text-[9px] font-black uppercase tracking-widest drop-shadow-md ${limitColor}">${current} / ${maxLabel} ITEMS</span></div>
                </div>
            </div>
