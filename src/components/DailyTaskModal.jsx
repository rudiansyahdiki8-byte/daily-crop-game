...
const DailyTaskModal = ({ isOpen, onClose, user, onClaimTask }) => {
  ...
  // Data progress user (dari backend)
  // Format user.dailyTasks = { 'LOGIN': true, 'HARVEST': true } atau tanggal
      ...
      <div style={{
        ...
        <div style={{flex:1, overflowY:'auto', padding:15, display:'flex', flexDirection:'column', gap:10}}>
           {DAILY_TASKS_LIST.map((task) => {
             // Cek apakah task sudah selesai hari ini
             // Asumsi: Backend menyimpan { 'LOGIN': '2023-10-27' }
               ...
               <div key={task.id} style={{
                 ...
                 <button 
                   ...
                   style={{
                     background: isDone 
                        ? 'transparent' 
                        : 'linear-gradient(90deg, #FF9800, #F57C00)', // Orange Button for Action
        ...
        <div style={{padding: 15, textAlign:'center', fontSize:'0.7rem', color:'#666', background:'rgba(0,0,0,0.2)'}}>
           Selesaikan misi & tonton iklan untuk mendapatkan koin gratis!
        </div>
      </div>
