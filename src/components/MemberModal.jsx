...
const MemberModal = ({ isOpen, onClose, currentPlan, onUpgrade, loading }) => {
  ...
  // KONFIGURASI TEMA PER PLAN (Supaya terlihat beda kelas)
  const PLAN_THEME = {
    FREE: { color: '#B0BEC5', icon: 'fa-user', label: 'BASIC', desc: 'Untuk Pemula' },
    MORTGAGE: { color: '#00E5FF', icon: 'fa-house-chimney', label: 'STARTER', desc: 'Mulai Bisnis Serius' },
    TENANT: { color: '#E040FB', icon: 'fa-building', label: 'POPULAR', desc: 'Pilihan Para Sultan' },
    OWNER: { color: '#FFD700', icon: 'fa-crown', label: 'VIP KING', desc: 'Kuasai Seluruh Pasar!' }
  };

  // Helper untuk hitung harga dalam PTS (untuk display estimasi)
  const rate = GAME_CONFIG.WITHDRAW?.RATE || 100000;
      ...
      <div style={{
        ...
        <div style={{
           ...
           <p style={{fontSize: '0.8rem', color: '#aaa', marginTop: 5}}>
              Upgrade statusmu untuk <span style={{color:'#FFD700'}}>Cuan Lebih Cepat!</span>
           </p>
        ...
        <div style={{flex: 1, padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 15}}>
          ...
          {Object.values(PLANS).map((plan) => {
             ...
             // Jangan tampilkan FREE di list pembelian jika user sudah upgrade (biar fokus ke yang mahal)
             if (isFree && !isCurrent) return null;
             ...
             return (
               <div key={plan.id} style={{
                  ...
                  {/* BADGE SUDUT (Opsional) */}
                  {plan.id === 'OWNER' && !isCurrent && (
                     <div style={{
                        position:'absolute', top:12, right:-25, background:'#FFD700', color:'black',
                        fontSize:'0.6rem', fontWeight:'bold', padding:'2px 30px', transform:'rotate(45deg)',
                        boxShadow:'0 2px 5px rgba(0,0,0,0.5)'
                     }}>BEST VALUE</div>
                  )}

                  <div style={{background: '#1a1a1a', borderRadius: 13, padding: 15, height: '100%'}}>
                      ...
                      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom: 10}}>
                          <div style={{display:'flex', gap: 10, alignItems:'center'}}>
                              <div style={{
                                  ...
                                  <i className={`fa-solid ${theme.icon}`} style={{color: theme.color, fontSize:'1.2rem'}}></i>
                              </div>
                              <div>
                                  <div style={{color: theme.color, fontWeight:'bold', fontSize:'1rem'}}>{plan.id}</div>
                          ...
                          {!isFree && (
                             <div style={{textAlign:'right'}}>
                                 <div style={{color:'white', fontWeight:'bold', fontSize:'1.2rem'}}>${plan.priceUsdt}</div>
                                 <div style={{color:'#555', fontSize:'0.6rem'}}>≈ {(plan.priceUsdt * rate).toLocaleString()} PTS</div>
                             </div>
                          )}
                          {isFree && <div style={{color:'#aaa', fontWeight:'bold'}}>FREE</div>}
                      </div>
                      ...
                      <div style={{
                          ...
                          <FeatureItem icon="fa-layer-group" txt={`${plan.plots} Petak Lahan`} highlight={true} />
                          <FeatureItem icon="fa-warehouse" txt={plan.storage === Infinity ? 'Unlimited' : `${plan.storage} Gudang`} />
                          <FeatureItem icon="fa-percent" txt={`+${(plan.bonusSell * 100)}% Bonus Jual`} color="#FFC107" />
                          <FeatureItem icon="fa-ban" txt={plan.ads === 'None' ? 'No Ads' : `${plan.ads} Ads`} color={plan.ads === 'None' ? '#4CAF50' : '#888'} />
                      </div>
                      ...
                      {isCurrent ? (
                          <div style={{
                              ...
                              <i className="fa-solid fa-check-circle"></i> PLAN SAAT INI
                          </div>
                      ) : (
                          <button 
                          ...
                          >
                             {loading ? 'Processing...' : `UPGRADE SEKARANG`}
                          </button>
                      )}
...
// Sub Component Kecil untuk List Fitur
