...
import { CONSUMABLES, ITEM_DETAILS, CROPS, EXTRA_SLOT_PRICE, STORAGE_UPGRADES, MAX_STORAGE_UPGRADES } from '../config/gameConstants';

// --- PRICE LOGIC (SAME AS BEFORE) ---
// (This logic is for visual display only, real price is validated by backend)
...
const MarketModal = ({ isOpen, onClose, user, onSellAll, onBuyItem, loading, onWatchAd }) => {
    ...
    // NEW STATE: For "Small Box" Single Sell Popup
    ...
    const storageUpgrades = user?.storageUpgradesPurchased || 0;
    const isMaxStorage = storageUpgrades >= MAX_STORAGE_UPGRADES;
                ...
                <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        ...
                        <ShopCard name={STORAGE_UPGRADES.STORAGE_20.name} price={STORAGE_UPGRADES.STORAGE_20.price} icon={STORAGE_UPGRADES.STORAGE_20.icon} color="#FFD700"
                            owned={isMaxStorage} buy={() => onBuyItem('STORAGE_20')} disabled={isMaxStorage} desc={`Capacity +${STORAGE_UPGRADES.STORAGE_20.capacity} (${storageUpgrades}/${MAX_STORAGE_UPGRADES})`} />
                    </div>
                </div>
                {/* TOOLS */}
                <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 15 }}>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
                        ...
                        >SELL ALL</button>
                        {isAdAvailable ? (
                            <button onClick={() => onSellAll(true)} disabled={loading || totalAssetValue === 0} style={{
                                background: totalAssetValue > 0 ? 'linear-gradient(90deg, #FF9800, #FF5722)' : '#333',
                                border: 'none', padding: '8px 16px', borderRadius: 10, color: 'white',
                                fontWeight: 'bold', fontSize: '0.75rem', cursor: totalAssetValue > 0 ? 'pointer' : 'not-allowed',
                                boxShadow: totalAssetValue > 0 ? '0 4px 10px rgba(255, 152, 0, 0.4)' : 'none'
                            }}
                            >🚀 SELL ALL + BOOST</button>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, flex: 1, overflow: 'hidden' }}>
                    ...
                    {/* MY INVENTORY (Right) - FIX: Added overflow:'hidden' to wrapper */}
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {/* === SMALL BOX (CUSTOM QUANTITY DIALOG) === */}
                {sellPopup && (
                    <div style={{
...
// ... ShopCard Component (Same as before, no changes needed)
