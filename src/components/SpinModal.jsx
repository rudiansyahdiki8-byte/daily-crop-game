...
const SpinModal = ({ isOpen, onClose, onSpin, userBalance, lastFreeSpin }) => {
  ...
  const handleSpinClick = async (mode) => {
    ...
    try {
      ...
      // --- SAFETY CHECK ---
      // If data is empty (e.g., connection lost), stop spinning to prevent sticking.
  ...
  const handleClaim = async () => {
    ...
    // If Free Spin -> Must Watch Ad
    if (pendingReward.mode === 'FREE') {
      const success = await showAdStack(1);
      if (!success) {
        setClaimLoading(false);
        return; // Ad failed/cancelled -> Stop claim
      }
      // Ad Success -> Call Claim API
      try {
      ...
      // Paid spin closes immediately (reward already added separately)
      setPendingReward(null);
      onClose();
    }
  };
