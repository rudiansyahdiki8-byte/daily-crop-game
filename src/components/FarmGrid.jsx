...
import { startFarming, harvestCrop, buyItem, harvestAll } from '../services/api';

const FarmGrid = ({ user, activePage, currentUserId, onRefreshUser, onOpenMember, onOpenWithdraw, setLoading }) => {

    // --- 1. STATE FOR TICKER (HEARTBEAT) ---
    // This makes the progress bar update every second
    const [, setTick] = useState(0); // eslint-disable-line no-unused-vars

    useEffect(() => {
        const interval = setInterval(() => {
            setTick(t => t + 1); // Force re-render every 1 second
        }, 1000);
    ...
    const getSlotStatus = (slotNum) => {
        ...
        // Shop Logic: Next slot after plan limit + extra
    ...
    const handleSlotClick = async (slotId) => {
        ...
        if (status === 'LOCKED_SHOP') {
            ...
            if (isConfirmed) {
                try {
                    ...
                    setLoading(false);
                    // Updated check to English "Balance"
                    if (e.response?.data?.message?.includes('Balance') || e.message?.includes('Balance')) {
        ...
        // -- MANUAL PLANT --
        // Only if slot is empty & user clicks manually (rare because of auto-replant)
        try {
            setLoading(true);
            if (!slotData) {
    ...
    const handleBatchHarvest = async () => {
        ...
        const now = Date.now();
        const readyCount = Object.values(farm).filter(s => s && now >= s.harvestAt).length;

        if (readyCount === 0) {
            await showRewardPopup("NO CROPS READY", "Wait for crops to grow!", "fa-clock");
            return;
        }
        ...
        const confirmed = await showConfirmPopup(
            "BATCH HARVEST",
            `Harvest ${readyCount} ready crops?\\nThis will show 1 ad for all.`,
            "fa-layer-group"
        );

        if (!confirmed) return;

        // Show ad (1 ad for all harvests)
        const adSuccess = await showAdStack(1);
        if (!adSuccess) {
            await showRewardPopup("AD REQUIRED", "Watch ad to harvest crops", "fa-film");
            return;
        }

        try {
            setLoading(true);
            const res = await harvestAll(currentUserId);
            await onRefreshUser();
            setLoading(false);

            await showRewardPopup(
                "BATCH HARVEST SUCCESS!",
                `Harvested ${res.totalCrops} crops (${res.totalItems} items)`,
                "fa-check-double"
            );
        } catch (e) {
            setLoading(false);
    ...
    // Current Time (Realtime because of Ticker)
    const now = Date.now();

    // Count ready crops for batch harvest button
    const farm = user?.farm || {};
    const readyCount = Object.values(farm).filter(s => s && now >= s.harvestAt).length;
    ...
    return (
        <div className="zone-farm">
            ...
            {user && pageSlots.map((slotNum) => {
                ...
                // Visual Progress Logic (Realtime)
                const isReady = s && now >= s.harvestAt;
                let progressPct = 0;
                let timeText = "";

                if (s && !isReady) {
                    const totalDuration = (s.harvestAt && s.plantedAt)
                        ? (s.harvestAt - s.plantedAt)
                        : 180000;
                ...
                if (visualStatus === 'ACTIVE' && s) {
                    if (isReady) {
                        overlayImage = '/assets/stage_growing.png'; // Can change to stage_ready.png if available
                        showBubble = true;
                    } else {
                        // Visual growth based on percent
