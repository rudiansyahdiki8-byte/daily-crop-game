...
import { ITEM_DETAILS, CONSUMABLES, PLANS, MAX_STORAGE_UPGRADES } from '../config/gameConstants';

const WarehouseModal = ({ isOpen, onClose, inventory, storageLimit, onOpenMarket, onUseItem, loading, userPlan, storageUpgradesPurchased }) => {
  ...
  const filteredList = inventoryEntries.filter(([name, qty]) => {
    ...
    if (activeTab === 'TOOLS') return isBooster;
    else return !isBooster;
  });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '5px 20px 10px' }}>
          ...
          {/* Storage Breakdown */}
          <div style={{ fontSize: '0.6rem', color: '#666', marginBottom: 5, textAlign: 'center' }}>
            {(() => {
              const baseCap = PLANS[userPlan || 'FREE']?.storage || 50;
              const extraCap = storageLimit - baseCap;
              const upgradeCount = storageUpgradesPurchased || 0;

              if (userPlan === 'OWNER') {
                return 'Unlimited Storage (OWNER Plan)';
              }

              return `Base: ${baseCap} | Upgrades: +${extraCap} (${upgradeCount}/${MAX_STORAGE_UPGRADES})`;
            })()}
          </div>
        ...
        <div style={{
          ...
          {filteredList.length === 0 ? (
            <div style={{ gridColumn: '1/4', textAlign: 'center', color: '#666', marginTop: 40, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <p style={{ fontSize: '0.9rem', marginTop: 10 }}>Empty</p>
            </div>
          ) : (
        ...
        <div style={{ padding: 15, background: 'rgba(0,0,0,0.3)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {activeTab === 'HARVEST' && (
            ...
            >
              <span style={{ fontSize: '1.4rem' }}>🛻</span> SELL AT MARKET
            </button>
          )}
          ...
          {activeTab === 'TOOLS' && (
            <div style={{ textAlign: 'center', fontSize: '0.75rem', color: '#03A9F4', background: 'rgba(3, 169, 244, 0.1)', padding: 10, borderRadius: 10, border: '1px solid rgba(3, 169, 244, 0.3)' }}>
              ℹ️ Item effect lasts for 24 Hours.
            </div>
          )}
