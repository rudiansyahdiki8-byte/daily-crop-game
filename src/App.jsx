...
import {
  ...
  buyItem, spinWheel, claimDailyTask, useItem as apiUseItem, bindUpline // <-- Import bindUpline
} from './services/api';
...
function App() {
  ...
  const handleBuyItem = async (itemId) => {
    ...
    } catch { setLoading(false); await showRewardPopup("ERROR", "Purchase Failed", "fa-xmark"); }
  };
  ...
  const handleUseItem = async (itemId) => {
    ...
    try {
      setLoading(true);
      await apiUseItem(currentUserId, itemId);
      await fetchUserData();
      setLoading(false);
      await showRewardPopup("ITEM ACTIVATED!", "Effect active for 24h", "fa-bolt");
    } catch { setLoading(false); await showRewardPopup("ERROR", "Failed to use item", "fa-xmark"); }
  };
  ...
  const handleWithdraw = async (amount, address, method) => {
    ...
    } catch { setLoading(false); await showRewardPopup("ERROR", "Withdraw Failed", "fa-xmark"); }
  };
  ...
  const renderActiveBuffs = () => {
    ...
    const now = Date.now();
    const activeBuffs = Object.entries(user.buffs).filter(([, expireTime]) => expireTime > now);
