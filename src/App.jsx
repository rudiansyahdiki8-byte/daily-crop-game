...
import {
  ...
  harvestCrop, startFarming // Added for Grid Logic
} from './services/api';

// --- COMPONENTS ---
// import FarmGrid from './components/FarmGrid'; // OLD
import PhaserGame from './components/PhaserGame'; // NEW PHASER
...
function App() {
        ...
        try {
          ...
          if (startParam && !userData.uplineId && String(startParam) !== String(realId)) {
            try {
              ...
              const updatedUser = await loginUser(realId, realUsername);
              setUser(updatedUser);
  ...
  // 2.5 HANDLER SLOT CLICK (MOVED FROM FarmGrid.jsx)
  const handleGridSlotClick = async (slotId) => {
    if (!currentUserId || !user) return;

    // Helper status (Refactored to shared utility)
    // const getSlotStatus ... removed

    const slotNum = parseInt(slotId.replace('slot', ''));
    const status = getSlotStatus(slotNum, user);

    // A. SHOP
    if (status === 'LOCKED_SHOP') {
      // Import EXTRA_SLOT_PRICE from constants if duplicating
      const EXTRA_SLOT_PRICE = { 1: 1000, 2: 75000 };
      ...
      const isConfirmed = await showConfirmPopup("EXPAND FARM", `Purchase Permanent Extra Land?\nPrice: ${price.toLocaleString()} PTS`, "fa-expand");
      if (isConfirmed) {
        try {
          setLoading(true);
          // Need to import buyItem but it is already imported in App.jsx
          ...
          setLoading(false);
          await showRewardPopup("ERROR", e.response?.data?.message || "Transaction Failed", "fa-xmark");
        }
      }
      return;
    }
    ...
    // B. LOCKED
    if (status === 'DISABLED') {
      const isUpgrade = await showConfirmPopup("AREA LOCKED", "This plot is locked by your Plan.\nUpgrade Membership to unlock?", "fa-lock");
      if (isUpgrade) setActiveModal('MEMBER');
      return;
    }
    ...
    if (slotData && Date.now() >= slotData.harvestAt) {
      if (await showAdStack(1)) {
        try {
          setLoading(true);
          // Need harvestCrop imported

          await harvestCrop(currentUserId, slotId);
          await fetchUserData();
          setLoading(false);
        } catch (e) { setLoading(false); await showRewardPopup("HARVEST FAILED", e.response?.data?.message || "Error", "fa-bug"); }
      }
      return;
    }
    // -- TANAM --
    if (!slotData) {
      try {
        ...
        await fetchUserData();
        setLoading(false);
      } catch (e) { setLoading(false); await showRewardPopup("PLANT FAILED", e.response?.data?.message || "Error", "fa-bug"); }
    }
  };
