...
async processSell(key, qty, price) {
    UIEngine.showToast("Trading assets...", "blue");

    try {
        // PANGGIL SERVER
        const result = await API.call('market/sell', {
            itemKey: key,
            amount: qty,
            isSellAll: false
        });
        ...
        // Feedback Sukses
        UIEngine.showRewardPopup("SOLD", `Sold ${qty} units for ${result.finalRevenue} PTS`, null, "OK");
        ...
        UIEngine.showRewardPopup("FAILED", e.message, null, "CLOSE");
    }
},

async sellAll() {
    UIEngine.showRewardPopup("LIQUIDATION", `Liquidate entire inventory?`, async () => {
        try {
            // PANGGIL SERVER
            const result = await API.call('market/sell', {
                isSellAll: true
            ...
            UIEngine.showRewardPopup("SUCCESS", `Revenue: ${result.finalRevenue} PTS`, null, "EXCELLENT");
...
async buyItem(id, price) {
    // Validasi lokal cepat (biar gak loading kalau uang jelas kurang)
    if (GameState.user.coins < price) { 
        UIEngine.showRewardPopup("INSUFFICIENT FUNDS", "Please add more funds.", null, "CLOSE");
        return;
    }

    const item = this.shopItems.find(i => i.id === id);
    
    UIEngine.showRewardPopup("CONFIRM", `Buy ${item.name} for ${price} PTS?`, async () => {
        try {
            // PANGGIL SERVER
            await API.call('market/buy', { itemId: id });

            UIEngine.showRewardPopup("SUCCESS", "Asset Acquired!", null, "OK");

            // Refresh Data User (Penting untuk update Buff/Tanah/Uang)
            const updatedUser = await API.loadUser();
            GameState.user = updatedUser;
