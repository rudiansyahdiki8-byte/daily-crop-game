...
async processSell(key, qty, price) {
        // 1. Hitung perkiraan harga visual (termasuk booster jika ada)
        // Pastikan Anda punya fungsi getSellMultiplier, jika tidak, pakai manual (this.isBoosterActive ? 1.2 : 1)
        const multiplier = (this.getSellMultiplier && typeof this.getSellMultiplier === 'function') 
                           ? this.getSellMultiplier() 
                           : (GameState.user.adBoosterCooldown > Date.now() ? 1.2 : 1.0);
        ...
        try {
            // 2. PANGGIL BACKEND (api/sell.js yang baru Anda buat)
            const response = await fetch('/api/sell', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: GameState.user.userId,
                    itemKey: key,       // Mengirim nama barang (misal: 'ginger')
                    qty: qty,           // Mengirim jumlah
                    clientPrice: finalPricePerUnit // Mengirim harga yang diharapkan user
                })
            });
            ...
            if (result.success) {
                // 3. JIKA SUKSES: Update Tampilan Frontend
                // Kurangi stok di tampilan
                GameState.warehouse[key] = (GameState.warehouse[key] || 0) - qty;
                if (GameState.warehouse[key] < 0) GameState.warehouse[key] = 0;

                // Tambah koin di tampilan (Visual saja, server sudah simpan yang asli)
                // Kita gunakan angka dari server (earned) jika ada, atau hitungan lokal
                const earned = result.earned || (finalPricePerUnit * qty);
                GameState.user.coins += earned; 
                GameState.user.totalSold = (GameState.user.totalSold || 0) + earned;
                ...
                // Simpan & Refresh Tampilan
                await GameState.save(); 
                UIEngine.updateHeader();
                this.renderSellInventory(); // Render ulang list barang
                this.calculatePreview();    // Hitung ulang total aset

                // Tampilkan Popup Sukses
                UIEngine.showRewardPopup("SOLD", `Berhasil menjual ${qty} unit seharga ${earned.toLocaleString()} PTS.`, null, "OK");
            } else {
                // GAGAL (Misal: Stok di server habis, atau harga dicurangi)
                UIEngine.showRewardPopup("FAILED", `Gagal: ${result.message}`, null, "TUTUP");
                
                // Opsional: Reload halaman jika dianggap curang agar data sinkron lagi
                // setTimeout(() => location.reload(), 2000);
            }

        } catch (error) {
            console.error("Sell Error:", error);
            UIEngine.showRewardPopup("CONNECTION ERROR", "Gagal menghubungi server.", null, "COBA LAGI");
        }
    },
 ...
 async sellAll() {
        ...
        if (totalPreview <= 0) {
            UIEngine.showRewardPopup("EMPTY", "Tidak ada barang untuk dijual.", null, "OK");
            return;
        }

        UIEngine.showRewardPopup("LIQUIDATION", `Jual semua aset senilai ±${totalPreview.toLocaleString()} PTS?`, async () => {
            
            UIEngine.showRewardPopup("PROCESSING", "Melakukan likuidasi aset...", null, "...");

            try {
                // PANGGIL BACKEND
                const response = await fetch('/api/market/sellall', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ userId: GameState.user.userId })
                });

                const result = await response.json();

                if (result.success) {
                    // Update Lokal (Bersihkan Gudang & Tambah Koin)
                    // Karena hitungan server mungkin beda dikit (karena harga max), 
                    // idealnya API return total. Tapi untuk cepat, kita pakai totalPreview lokal.
                    Object.keys(GameState.warehouse).forEach(k => { GameState.warehouse[k] = 0; });
                    GameState.user.coins += totalPreview;
                    GameState.user.totalSold += totalPreview;
                    ...
                    UIEngine.showRewardPopup("SUCCESS", "Likuidasi Berhasil!", null, "MANTAP");
                } else {
    ...
    async buyItem(id, price) {
        // Cek visual saldo dulu biar cepat
        if (GameState.user.coins < price) { 
            UIEngine.showRewardPopup("INSUFFICIENT FUNDS", "Saldo tidak cukup.", null, "CLOSE");
            return;
        }

        const item = this.shopItems.find(i => i.id === id);
        
        UIEngine.showRewardPopup("CONFIRM ACQUISITION", `Beli ${item.name} seharga ${price.toLocaleString()} PTS?`, async () => {
            
            UIEngine.showRewardPopup("PROCESSING", "Sedang memproses pembelian...", null, "...");
            ...
            try {
                ...
                if (result.success) {
                    // Update State Lokal (Agar visual langsung berubah tanpa reload)
                    GameState.user.coins -= price;
                    GameState.user.totalSpent = (GameState.user.totalSpent || 0) + price;
                    ...
                    UIEngine.updateHeader();
                    this.renderShopItems();
                    UIEngine.showRewardPopup("SUCCESS", "Item berhasil dibeli!", null, "OK");
                } else {
                    UIEngine.showRewardPopup("FAILED", result.message, null, "CLOSE");
                }
            } catch (e) {
                console.error(e);
                UIEngine.showRewardPopup("ERROR", "Gagal menghubungi server", null, "RETRY");
            }
...
applyPriceBooster() {
        UIEngine.showRewardPopup("MARKET LEVERAGE", "Nonton iklan untuk boost harga +20% selama 24 jam?", async () => {
            // Panggil Iklan
            AdsManager.showHybridStack(3, async () => {
                
                // PANGGIL BACKEND UNTUK AKTIFKAN BOOSTER
                try {
                    await fetch('/api/market/boost', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ userId: GameState.user.userId })
                    });
                    ...
                    this.checkBoosterStatus();
                    this.calculatePreview();
                    UIEngine.showRewardPopup("LEVERAGE ACTIVE", "Harga pasar naik 20%!", null, "TRADE NOW");
                } catch (e) {
                    console.error("Gagal aktivasi boost server");
                }
    ...
    checkBoosterStatus() {
        const now = Date.now();
        // Cek apakah waktu kedaluwarsa booster di server masih berlaku
        const isBoostActive = GameState.user.adBoosterCooldown && GameState.user.adBoosterCooldown > now;
        
        const btn = document.getElementById('market-booster-btn');
        if (btn) {
            if (isBoostActive) {
                btn.innerHTML = '<i class="fas fa-rocket"></i> BOOSTER ACTIVE (24H)';
                btn.classList.replace('bg-amber-500', 'bg-green-500');
                btn.onclick = null; // Matikan klik jika sudah aktif
            } else {
                btn.innerHTML = '<i class="fas fa-ad"></i> ACTIVATE PRICE BOOSTER (+20%)';
