...
const PlanSystem = {
    ...
    async redirectToDeposit(planKey, priceUsdt) {
            // Hitung estimasi harga koin (sesuai config server)
            const rate = window.GameConfig.Finance.RateUSDT || 0.00001;
            const priceCoins = priceUsdt / rate;

            // Cek Saldo Lokal Dulu
            if (GameState.user.coins >= priceCoins) {
                UIEngine.showRewardPopup(
                    "UPGRADE PLAN", 
                    `Beli ${planKey} seharga ${priceCoins.toLocaleString()} PTS?`, 
                    async () => {
                        // --- PANGGIL BACKEND ---
                        UIEngine.showRewardPopup("PROCESSING", "Mengupgrade akun...", null, "...");
                        ...
                        try {
                            ...
                            if (result.success) {
                                // Update Lokal
                                GameState.user.coins -= priceCoins;
                                GameState.user.plan = planKey;
                                await GameState.save();
                                
                                // Refresh UI
                                UIEngine.updateHeader();
                                PlanSystem.init(); // Render ulang kartu plan
                                UIEngine.showRewardPopup("SUCCESS", result.message, null, "MANTAP");
                            } else {
                                UIEngine.showRewardPopup("FAILED", result.message, null, "TUTUP");
                            }
                        } catch (e) {
                            console.error(e);
                ...
                // Jika saldo kurang, arahkan ke Deposit (Logic Lama)
                UIEngine.showRewardPopup(
                    "INSUFFICIENT FUNDS", 
                    `Butuh ${priceCoins.toLocaleString()} PTS. Saldo kurang. Ke menu Deposit?`, 
                    () => {
                        UIEngine.openWithdraw();
                        if(window.WithdrawSystem) WithdrawSystem.switchTab('deposit');
                    }, 
                    "DEPOSIT"
                );
            }
