// js/plan.js
const PlanSystem = {
    // Data Visual (Tetap di Frontend karena ini gambar & teks)
    tierVisuals: {
        'FREE': { 
    ...
    async init() {
        ...
        if (container) {
            // Tampilkan Loading
            container.innerHTML = `<div class="flex items-center justify-center h-full text-emerald-500 animate-pulse"><i class="fas fa-circle-notch fa-spin text-2xl"></i></div>`;
            
            try {
                // MINTA DATA KE SERVER
                const response = await fetch(`/api/plans?userId=${GameState.user.userId}`);
                const data = await response.json();

                if (data.success) {
                    this.render(container, data.plans);
                }
            } catch (e) {
                console.error("Failed to load plans:", e);
                container.innerHTML = `<div class="flex items-center justify-center h-full text-red-400">Connection Error</div>`;
            }
        }
    },

    render(container, plansData) {
        container.innerHTML = '';
        ...
        plansData.forEach((plan, index) => {
            const visual = this.tierVisuals[plan.id]; // Ambil data gambar/teks
            const status = plan.status; // 'active', 'available', 'locked', 'owned'
            ...
            // LOGIKA TAMPILAN BERDASARKAN STATUS DARI SERVER
            if (status === 'active') {
                // PLAN SAAT INI
                ...
                // LEVEL LEBIH RENDAH (Sudah lewat)
                ...
                // TERKUNCI (Loncat level)
                cardBaseClass += ` ${visual.bg} ${visual.border} opacity-50 grayscale`; // Visual agak gelap
                rightHTML = `
                <button onclick="UIEngine.showRewardPopup('LOCKED', 'Please upgrade to previous level first.', null, 'OK')" class="w-full h-full flex flex-col items-center justify-center bg-black/40 gap-1 text-gray-400 cursor-not-allowed">
                ...
                if(plan.id === 'OWNER') {
                    ...
                    cardBaseClass += ` ${visual.bg} ${visual.border}`;
                }
                
                // TOMBOL UPGRADE AKTIF
                rightHTML = `
                <button onclick="PlanSystem.redirectToDeposit('${plan.id}', ${price})" class="w-full h-full flex flex-col items-center justify-center bg-black/20 hover:bg-black/10 transition-colors gap-1 text-white relative overflow-hidden group">
            ...
            // RENDER FITUR LIST
            const featuresHTML = visual.desc.map(d => `<div class="flex items-center gap-2 text-white/90"><div class="w-4 flex justify-center"><i class="fas ${d.i} text-[10px] ${d.c}"></i></div><span class="text-[9px] font-bold uppercase tracking-wide text-gray-200 shadow-black drop-shadow-sm truncate">${d.t}</span></div>`).join('');
            
            // BENTUK KARTU
            const card = document.createElement('div');
            card.className = cardBaseClass;
            card.style = animDelay;
            card.innerHTML = `${overlayHTML}<div class="flex-1 p-3 flex flex-col justify-between relative z-10 pl-4"><div class="border-b border-white/10 pb-2 mb-1"><div class="text-[9px] font-black text-amber-400 uppercase tracking-widest mb-0.5">${visual.sub}</div><h3 class="text-xl font-black text-white italic uppercase tracking-wider leading-none drop-shadow-md">${visual.title}</h3></div><div class="grid grid-cols-1 gap-1">${featuresHTML}</div></div><div class="w-20 border-l border-black/20 relative z-20 flex-shrink-0 backdrop-blur-sm">${rightHTML}</div><div class="absolute -bottom-2 right-20 text-7xl text-white opacity-5 pointer-events-none z-0 rotate-12"><i class="fas ${visual.desc[0].i.replace('fa-', 'fa-')}"></i></div>`;
    ...
    redirectToDeposit(planKey, price) {
        // Validasi lagi di UI (Sekadar UX, keamanan asli sudah di server yang memberikan status 'available')
