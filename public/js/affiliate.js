...
const AffiliateSystem = {
 ...
 async checkReferralParam() {
        ...
        // Cek Lokal dulu biar hemat request
        if (refCode && !GameState.user.upline && refCode !== GameState.user.userId) {
            
            console.log(`[AFFILIATE] Found Ref Code: ${refCode}, Binding to server...`);
            ...
            try {
                ...
                if(result.success) {
                    GameState.user.upline = refCode; // Update lokal
                    console.log("Success Binding Upline");
                } else {
                    console.warn("Bind Failed:", result.error);
                }
            } catch (e) {
