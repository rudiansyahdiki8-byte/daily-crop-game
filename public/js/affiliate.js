// js/affiliate.js
// ==========================================
// AFFILIATE SYSTEM (SERVER-SIDE SECURED)
// Update: Logic pengikatan upline dipindah ke API Server
// ==========================================
...
const AffiliateSystem = {
    ...
    // Menangkap "startapp=USERID" dari Telegram/URL
    async checkReferralParam() {
        // Jika sudah punya upline di state lokal, tidak perlu cek lagi
        if (GameState.user.upline) return;

        const urlParams = new URLSearchParams(window.location.search);
        const refCode = urlParams.get('startapp') || urlParams.get('ref');
        
        // Cek validasi dasar di client (Validasi ketat tetap di server)
        if (refCode && refCode !== GameState.user.userId) {
            
            console.log(`[AFFILIATE] Found referral code: ${refCode}. Requesting bind...`);
            ...
            try {
                ...
                if (result.success) {
                    // Berhasil: Update State Lokal
                    GameState.user.upline = refCode;
                    await GameState.save(); // Simpan info upline ke cache lokal
                    
                    if(window.UIEngine) UIEngine.showRewardPopup("REFERRAL SUCCESS", "You have been linked to your inviter!", null, "OK");
                    console.log(`[AFFILIATE] Successfully bound to: ${refCode}`);
                } else {
                    console.warn(`[AFFILIATE] Failed to bind: ${result.message}`);
                }
    ...
    render() {
        ...
        // Ambil Data dari State (Data yang sudah dimuat dari Firebase Server)
                ...
                <div class="glass p-3 rounded-2xl flex items-center justify-between border border-white/5 mb-2">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-black text-[10px] text-white">
                            ${f.name ? f.name.substring(0, 2).toUpperCase() : '??'}
                        </div>
                        <div>
                            <p class="text-[9px] font-black text-white uppercase">${f.name || 'Unknown'}</p>
                            <p class="text-[7px] font-bold text-emerald-400">Active Member</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-[9px] font-black text-white">+${(f.earnings || 0).toLocaleString()} PTS</p>
