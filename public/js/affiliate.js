...
const AffiliateSystem = {
    ...
    async checkReferralParam() {
        ...
        // Cek lokal dulu biar gak spam API
        if (refCode && !GameState.user.upline && refCode !== GameState.user.userId) {
            console.log("Found Referral:", refCode);
            
            try {
                // Panggil API untuk proses di server
                const res = await API.call('user/referral', { refCode: refCode });
                
                if (res && res.success) {
                    // Update state lokal
                    GameState.user.upline = refCode;
                    // Tidak perlu save() manual karena server sudah update
                }
            } catch (e) {
