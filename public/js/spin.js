...
const SpinSystem = {
 ...
 async start(type) {
        ...
        // 1. VALIDASI PRE-FLIGHT (Cek Saldo/Cooldown di Client biar cepat)
        if (type === 'paid') {
            const cost = window.GameConfig.Spin.CostPaid;
            if (GameState.user.coins < cost) {
                UIEngine.showRewardPopup("NO COINS", `Need ${cost} PTS to spin.`, null, "CLOSE");
            ...
            // Langsung Gas ke Server
            this.callSpinAPI(type);
        } 
        else if (type === 'free') {
            // Cek Cooldown
            const now = Date.now();
            const lastSpin = GameState.user.spin_free_cooldown || 0;
            const cdTime = window.GameConfig.Spin.CooldownFree;
            
            if (now < lastSpin + cdTime) {
                UIEngine.showRewardPopup("COOLDOWN", "Please wait for cooldown.", null, "OK");
                return;
            }

            // UX SECURE: Tonton Iklan DULU baru boleh Spin
            UIEngine.showRewardPopup("FREE SPIN", "Watch Ad to Spin for Free?", () => {
                AdsManager.showAd('reward', () => {
                    // Jika iklan sukses ditonton, baru panggil Server
                    this.callSpinAPI('free');
                });
            }, "WATCH AD");
        }
    },

    async callSpinAPI(type) {
        ...
        try {
            ...
            if (result.success) {
                // Server sudah menentukan 'targetIndex'. Kita tinggal animasi ke sana.
                this.runWheelAnimation(result.targetIndex, result.rewardText, type);
                
                // Update State (Koin berkurang/Cooldown update)
                // Kita load background agar seamless
                GameState.load(); 
            } else {
                this.isSpinning = false;
                UIEngine.showRewardPopup("ERROR", result.error, null, "CLOSE");
                this.updateTimer(); // Reset tombol jika error
            }

        } catch (e) {
            this.isSpinning = false;
            console.error(e);
            UIEngine.showRewardPopup("CONNECTION", "Network Error", null, "RETRY");
        }
    },

    runWheelAnimation(targetIndex, rewardName, type) {
        const wheel = document.getElementById('real-wheel');
        
        // Hitung Rotasi Visual (Logic Visual Lama Anda [cite: 137-140])
        const segmentAngle = 360 / 8;
        const randomOffset = Math.floor(Math.random() * 20) - 10; 
        const spinRounds = 5 * 360; // 5 Putaran Penuh
        
        // Target Rotasi berdasarkan Index dari Server
        ...
        setTimeout(() => {
            ...
            UIEngine.showRewardPopup(
                type === 'paid' ? "PURCHASED" : "LUCKY SPIN", 
                `<div class="flex flex-col items-center gap-2">
                    <span class="text-3xl animate-bounce">🎁</span>
                    <span class="text-lg font-black text-white uppercase">YOU GOT ${rewardName}!</span>
                    <span class="text-[9px] text-gray-400">Reward added to inventory</span>
                </div>`, 
                null, 
                "AWESOME"
            );
