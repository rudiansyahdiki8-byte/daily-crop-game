...
async start(type) {
    ...
    // UI Check Cepat
    if (type === 'paid' && GameState.user.coins < window.GameConfig.Spin.CostPaid) {
        UIEngine.showRewardPopup("NO COINS", "Need more funds.", null, "CLOSE");
        return;
    }

    // 1. PANGGIL API SPIN (Minta hasil dari server)
    try {
        const result = await API.call('game/spin', { type: type });
        
        // Server sudah mengurangi koin/update cooldown & memberikan hadiah
        // Kita simpan data hasil ini untuk ditampilkan SETELAH animasi selesai
        this.pendingResult = result; 
        
        // Update Koin Visual Sementara (agar user lihat koin berkurang)
        if (type === 'paid') GameState.user.coins -= window.GameConfig.Spin.CostPaid;
        
        // 2. JALANKAN ANIMASI VISUAL
        // Kita kirim Index Target (ID) yang didapat dari server ke fungsi animasi
        this.executeSpin(result.prize.id, type);

    } catch (e) {
        UIEngine.showRewardPopup("ERROR", e.message, null, "CLOSE");
    }
},

executeSpin(targetIndex, type) {
    ...
    // Hitung sudut agar jarum berhenti di targetIndex yang diberi Server
    ...
    setTimeout(async () => {
        ...
        // Refresh data user lengkap dari server (Sync saldo & item)
        ...
        // Tampilkan Hadiah
        const prize = this.pendingResult.prize;
        let msg = `You got ${prize.type.toUpperCase()}`;
        if(prize.val) msg += ` ${prize.val}`;
        if(prize.key) msg += ` ${prize.key}`;

        UIEngine.showRewardPopup("SPIN RESULT", msg, null, "NICE");
