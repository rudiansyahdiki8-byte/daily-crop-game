...
const UIEngine = {
    ...
    navigate(pageId) {
        // Hapus pengecekan GameState agar navigasi tetap jalan walau data error
        // if (!window.GameState || !GameState.isLoaded) return; <-- HAPUS BARIS INI (Optional untuk debugging)

        // 1. SEMBUNYIKAN SEMUA HALAMAN (Reset Mutlak)
        this.pages.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.classList.add('hidden');
                el.classList.remove('flex'); // Pastikan flex juga hilang
            }
        });
        ...
        if (target) {
            ...
            // Tambahkan flex agar layout rapi (karena Tailwind butuh display:flex untuk layout kolom)
            target.classList.add('flex'); 
            
            target.classList.remove('animate-in');
            void target.offsetWidth; // Trigger reflow untuk restart animasi
        ...
        // Trigger Sub-Systems (Biarkan kode asli [cite: 278-280])
