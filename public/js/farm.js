...
// GANTI FUNGSI INI DI DALAM public/js/farm.js
    ...
    handleTaskClick(task, btnElement) {
        ...
        // Panggil Iklan Dulu (Logic Client Side, Server tidak peduli iklannya, yang penting cooldownnya)
            ...
            try {
                ...
                const response = await fetch('/api/task', {
                    ...
                    body: JSON.stringify({
                        ...
                        taskId: task.id // Kirim ID tugas (misal 'daily_login')
                    })
                });
                ...
                if (result.success) {
                    // SERVER ACC: Update UI
                    GameState.user.coins = result.newCoins;
                    
                    // Update Cooldown Lokal (Hanya visual, validasi tetap di server)
                    ...
                    this.renderTaskButtons(); // Refresh tombol jadi abu-abu
  ...
  // GANTI FUNGSI INI DI DALAM public/js/farm.js
    ...
    async plantAll() {
        UIEngine.showRewardPopup("PLANTING", "Sowing seeds...", null, "...");
        ...
        try {
            ...
            if (result.success) {
                // Update Data Lokal dengan Data Server (Yang sudah ada RNG & Timer valid)
                GameState.farmPlots = result.farmPlots;
                
                await GameState.save();
                this.renderFarmGrid();
                
                UIEngine.showRewardPopup("PLANTED", `Planted ${result.plantedCount} seeds!`, null, "GROW!");
            } else {
                UIEngine.showRewardPopup("FULL", result.message || "No empty plots.", null, "OK");
            }
...
// GANTI FUNGSI INI DI DALAM public/js/farm.js
    ...
    async executeHarvestLogic(plots) {
        ...
        // Loop untuk memanen setiap plot yang dipilih
        for (const plot of plots) {
            // Cari nomor index plot
            ...
            try {
                ...
                if (result.success) {
                    // SERVER OK: Update Tampilan Game
                    
                    // 1. Update Plot Lokal dengan data dari Server
                    GameState.farmPlots = result.data.farmPlots;

                    // 2. Update Gudang Lokal
                    const item = result.data.addedItem;
                    const amount = result.data.addedAmount;
                    
                    if (!GameState.warehouse) GameState.warehouse = {};
                    GameState.warehouse[item] = result.data.newStock;
                    
                    GameState.user.totalHarvest += amount;
                    
                    // 3. Mainkan Animasi Terbang (Visual)
                    const plantImgUrl = `assets_iso/plant_${item.toLowerCase()}.png`;
                    this.playFlyAnimation(plantImgUrl, originalIndex);

                    successCount++;
                    totalItems += amount;
                } else {
                    console.warn("Gagal Panen:", result.message);
                }
            } catch (err) {
        ...
        // Simpan state terbaru ke lokal biar aman
        if (GameState.save) await GameState.save();
        ...
        // Lapor Hasil ke User
        if (successCount > 0) {
            UIEngine.showRewardPopup("HARVEST SUCCESS", `Harvested ${totalItems} items & Auto-Replanted!`, null, "GREAT");
        } else {
            UIEngine.showRewardPopup("FAILED", "Harvest failed. Check connection or timer.", null, "CLOSE");
        }
    },
