...
async plantAll() {
    // 1. UI Feedback (Biar user tahu sedang loading)
    UIEngine.showToast("Planting seeds...", "blue");

    try {
        // 2. Panggil API (Kirim request ke Vercel)
        // Parameter kosong {} karena kita minta "Plant All"
        const result = await API.call('farm/plant', {});

        // 3. Jika Sukses, Refresh Data User
        // Kita panggil API Load User lagi untuk update tampilan kebun
        // (Ini cara paling aman agar tampilan sinkron dengan server)
        const updatedUser = await API.loadUser();
        
        // Update State Lokal
        GameState.user = updatedUser;
        GameState.farmPlots = updatedUser.farmPlots || [];
        
        // Konversi Timestamp Firestore string kembali ke miliseconds untuk timer JS
        // API biasanya mengembalikan Date string atau Timestamp object
        GameState.farmPlots.forEach(plot => {
           if(plot.harvestAt && typeof plot.harvestAt === 'string') {
               plot.harvestAt = new Date(plot.harvestAt).getTime();
           } else if (plot.harvestAt && plot.harvestAt.seconds) {
        ...
        // 4. Render Ulang
        this.renderFarmGrid();
        UIEngine.showRewardPopup("PLANTED", `Seeds planted successfully!`, null, "GROW!");

    } catch (error) {
        // Handle Error (Misal: Tanah Penuh)
        if (error.message.includes("No empty plots")) {
            UIEngine.showRewardPopup("FULL", "No empty unlocked plots available.", null, "OK");
        } else {
            UIEngine.showRewardPopup("ERROR", error.message, null, "CLOSE");
        }
    }
},
    ...
    async harvestAll(specificIndex = null) {
        // UI Feedback
        UIEngine.showToast("Harvesting...", "blue");

        try {
            // 1. Panggil API Server
            const result = await API.call('farm/harvest', {});

            // 2. Feedback Visual (Animasi terbang item)
            // Kita pakai data dari server: "harvestedItems"
            const items = result.harvestedItems || {}; // Contoh: { ginger: 2, chili: 1 }
            
            // Tampilkan pesan sukses
            let msg = "Harvested: ";
            for (const [name, count] of Object.entries(items)) {
                msg += `${count} ${name} `;
                // (Opsional) Trigger animasi terbang di sini jika mau detail
            }
            UIEngine.showToast(msg, "green");

            // 3. Auto Replant (Opsional - Agar gameplay mulus)
            // Setelah panen sukses (tanah jadi kosong), kita langsung minta tanam lagi
            await this.plantAll(); 

        } catch (error) {
            if (error.message.includes("Nothing to harvest")) {
                UIEngine.showToast("Belum ada yang siap panen!", "red");
            } else {
                UIEngine.showToast("Gagal Panen: " + error.message, "red");
            }
        }
        
        // 4. Update Data Lokal (PENTING: Ambil data terbaru dari server)
        const updatedUser = await API.loadUser();
        GameState.user = updatedUser;
        GameState.farmPlots = updatedUser.farmPlots || [];
        GameState.warehouse = updatedUser.warehouse || {};
        
        // Fix Timestamp untuk timer JS
        GameState.farmPlots.forEach(plot => {
        if(plot.harvestAt && typeof plot.harvestAt === 'string') {
            plot.harvestAt = new Date(plot.harvestAt).getTime();
        } else if (plot.harvestAt && plot.harvestAt.seconds) {
