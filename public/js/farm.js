...
handleTaskClick(task, btnElement) {
        ...
        // Tampilkan Iklan dulu (Sesuai logic asli Anda )
        AdsManager.showAd('reward', async () => {
            ...
            try {
                ...
                if(result.success) {
                    ...
                    await GameState.load(); // Sync koin & task status
                    this.renderTaskButtons();
    ...
    async executeHarvestLogic(plots) {
        // Tampilkan Loading
        if(window.UIEngine) UIEngine.showRewardPopup("HARVESTING", "Syncing with server...", null, "...");

        // Ambil index plot yang mau dipanen (untuk dikirim ke server)
        // Kita cari index asli plot di array GameState.farmPlots
        const indexesToHarvest = plots.map(p => GameState.farmPlots.indexOf(p));

        try {
            const response = await fetch('/api/farm/harvest', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: GameState.user.userId,
                    plotIndexes: indexesToHarvest
                })
            });
            ...
            if (result.success) {
                // SUKSES: Update data lokal agar sama dengan server
                await GameState.load(); // Reload data terbaru
                
                this.renderFarmGrid(); // Gambar ulang kebun
                UIEngine.updateHeader(); // Update header (jika ada stats berubah)
                
                UIEngine.showRewardPopup("HARVEST SUCCESS", `Harvested items & Replanted!`, null, "GREAT");
            } else {
                UIEngine.showRewardPopup("FAILED", `Gagal Panen: ${result.error}`, null, "OK");
            }
        } catch (error) {
