...
// Update di public/js/farm.js
    ...
    handleTaskClick(task, btnElement) {
        ...
        // Panggil Iklan Dulu (Syarat Wajib)
        AdsManager.showHybridStack(3, async () => {
            
            // --- MULAI LOGIKA BACKEND ---
            UIEngine.showRewardPopup("CLAIMING", "Verifying task...", null, "...");
            
            try {
                const response = await fetch('/api/farm/actions', {
                    method: 'POST',
                ...
                if (result.success) {
                    // Update Saldo Lokal (Visual)
                    GameState.user.coins += result.reward;
                    
                    // Update Cooldown Lokal
                    ...
                    // Simpan State & Render Ulang Tombol
                    await GameState.save(); 
                    this.renderTaskButtons();
                    UIEngine.showRewardPopup("SUCCESS", result.message, null, "OK");
                } else {
            ...
            // --- SELESAI LOGIKA BACKEND ---

        });
    },

    async plantAll() {
        UIEngine.showRewardPopup("PROCESSING", "Sedang menanam...", null, "...");
        ...
        try {
            ...
            if (result.success) {
                // Update Lokal dengan data dari Server (Supaya sinkron waktunya)
                GameState.farmPlots = result.plots;
                await GameState.save();
                this.renderFarmGrid();
                UIEngine.showRewardPopup("PLANTED", result.message, null, "GROW!");
            } else {
                UIEngine.showRewardPopup("FAILED", result.message, null, "CLOSE");
            }
        } catch (e) {
    ...
    async executeHarvestLogic(plots) {
        // Kita panen satu persatu atau ubah API support array. 
        // Untuk simpelnya, kita loop di frontend, tapi idealnya API support 'harvestAll'.
        // Mari kita modifikasi sedikit agar loop aman.

        let successCount = 0;

        // Tampilkan loading sekali saja
        // UIEngine.showRewardPopup... (Optional)

        for (const plot of plots) {
            const index = GameState.farmPlots.indexOf(plot);
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Update Gudang Lokal (Visual)
                    const cropName = plot.plant || 'ginger';
                    GameState.warehouse[cropName] = (GameState.warehouse[cropName] || 0) + (result.yield || 1);

                    // Animasi Visual
                    const plantImgUrl = `assets_iso/plant_${cropName.toLowerCase()}.png`;
                    this.playFlyAnimation(plantImgUrl, index);
        ...
        if (successCount > 0) {
            await GameState.save();
            this.renderFarmGrid();
            UIEngine.showRewardPopup("HARVEST", `Berhasil panen ${successCount} tanaman & Replanted!`, null, "OK");
        }
    },
    startEngine() {
