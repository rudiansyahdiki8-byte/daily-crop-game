...
async process() {
        ...
        const amountPTS = parseInt(amountStr);

        // 1. Validasi Input Dasar di Frontend
        if (!address || address.length < 5) {
            UIEngine.showRewardPopup("INVALID", "Please enter a valid wallet address.", null, "RETRY");
            return;
        }

        if (isNaN(amountPTS) || amountPTS < 100) { // Misal minimal 100 koin
            UIEngine.showRewardPopup("INVALID", "Minimum withdrawal is 100 PTS.", null, "RETRY");
            return;
        }

        // 2. Cek Saldo Lokal (Hanya untuk kenyamanan user)
        if (GameState.user.coins < amountPTS) {
            UIEngine.showRewardPopup("INSUFFICIENT", "You don't have enough balance.", null, "CLOSE");
            return;
        }

        UIEngine.showRewardPopup("CONFIRM TRANSFER", `Initiate transfer?`, async () => {
            UIEngine.showRewardPopup("PROCESSING", "Menghubungi Server...", null, "...");

            try {
                // MEMANGGIL BACKEND YANG KITA BUAT DI ATAS
                ...
                if (result.success) {
                    // Update tampilan UI saja (Saldo asli sudah dikurangi di server)
                    ...
                    await GameState.save(); // Simpan state lokal agar sinkron
                    UIEngine.updateHeader();
                    UIEngine.closeWithdraw();
                    UIEngine.showRewardPopup("SUKSES", "Dana telah dikirim via FaucetPay.", null, "OK");
                } else {
                    UIEngine.showRewardPopup("GAGAL", `Error: ${result.message}`, null, "TUTUP");
                }
            } catch (error) {
                console.error("Network Error:", error);
                UIEngine.showRewardPopup("ERROR", "Gagal menghubungi server.", null, "COBA LAGI");
            }
        }, "KIRIM SEKARANG");
    },
};
