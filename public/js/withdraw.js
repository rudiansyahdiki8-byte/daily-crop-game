...
UIEngine.showRewardPopup("CONFIRM TRANSFER", `Initiate transfer?`, async () => {
        ...
        UIEngine.showRewardPopup("PROCESSING", "Securing Transaction...", null, "...");

        try {
            // [UPDATE] MENGGUNAKAN API.call (Jembatan aman kita)
            // Kita tidak pakai fetch manual agar initData otomatis terkirim
            
            const result = await API.call('withdraw', {
                address: address,
                amount: amountPTS,
                currency: this.selectedCurrency
            });

            // Jika sampai sini, berarti Backend sukses (success: true)
            
            // Update State Lokal
            GameState.user.coins = result.new_balance; // Update saldo dari server
            if (!GameState.user.has_withdrawn) {
                GameState.user.has_withdrawn = true;
                GameState.user.faucetpay_email = address;
            }

            // Catat History di Lokal (Visual saja)
            const tx = { 
                id: result.payout_id, 
                ...
                destination: address, 
                status: 'Processing' // Status server
            };
            
            if(!GameState.user.history) GameState.user.history = [];
            GameState.user.history.unshift(tx);

            // Simpan perubahan lokal (agar UI update)
            // Tidak perlu save() ke server karena server sudah update DB sendiri
            UIEngine.updateHeader();
            UIEngine.closeWithdraw();
            
            UIEngine.showRewardPopup("REQUEST RECEIVED", "Withdrawal has been queued securely.", null, "EXCELLENT");

        } catch (error) {
            console.error("API Error:", error);
            UIEngine.showRewardPopup("TRANSACTION FAILED", `Server Rejection: ${error.message}`, null, "CHECK WALLET");
        }
