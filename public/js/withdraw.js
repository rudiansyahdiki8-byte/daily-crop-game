// js/withdraw.js
// ==========================================
// FINANCE SYSTEM (REALTIME RATES + FAUCETPAY)
// Update: Menggunakan API Server untuk keamanan transaksi
// ==========================================
...
const WithdrawSystem = {
    ...
    // SAFETY MARGIN 5% (Visual Only - Server yang menentukan finalnya)
    SAFETY_MARGIN: 0.95, 
    ...
    init() {
        ...
        this.render();
        // Langsung ambil harga saat menu dibuka
        this.fetchLiveRates();
    },

    // --- AMBIL HARGA REALTIME (Visual Preview) ---
    ...
    // --- FUNGSI PROSES WD (UPDATED KE API SERVER) ---
    async process() {
        const inputAmt = document.getElementById('wd-amount');
        ...
        if (!amountPTS || amountPTS < minLimit) { 
            UIEngine.showRewardPopup("VALIDATION ERROR", `Minimum required: ${minLimit} PTS`, null, "FIX"); 
            return;
        }
        if (!address) { 
            UIEngine.showRewardPopup("MISSING DATA", "Please provide a valid destination address.", null, "FIX");
            return; 
        }
        ...
        // Cek kecocokan email FaucetPay jika sudah terikat (Visual Check)
        ...
        UIEngine.showRewardPopup("CONFIRM TRANSFER", `Initiate transfer of ~${estimatedCrypto} ${this.selectedCurrency} to ${address}?`, async () => {
            ...
            try {
                // --- PANGGIL API SERVER (AMANKAN TRANSAKSI) ---
                ...
                if (result.success) {
                    // BERHASIL: Update UI dengan data dari server
                    ...
                    // Tambah History Lokal (Server juga sudah catat)
                    const tx = { 
