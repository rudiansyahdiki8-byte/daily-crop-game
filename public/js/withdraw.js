...
const WithdrawSystem = {
    ...
    rates: {
        ...
        TON:  0.0002, TRX: 0.06, LTC: 0.00001, DOGE: 0.03, SOL: 0.00005, BTC: 0.00000002
    },

    coinIds: {
        'TON': 'the-open-network', 'TRX': 'tron', 'LTC': 'litecoin', 
        'DOGE': 'dogecoin', 'SOL': 'solana', 'BTC': 'bitcoin', 'USDT': 'tether'
    },
    ...
    renderButtons() {
        Object.keys(this.rates).forEach(k => {
            ...
            if(btn) {
                btn.className = (k === this.selectedCurrency) 
                    ? "p-2 glass rounded-2xl flex flex-col items-center border border-emerald-500/50 bg-emerald-500/10 scale-95 transition-all"
                    : "p-2 glass rounded-2xl flex flex-col items-center border border-white/5 opacity-70 hover:bg-white/5 transition-all";
            }
        });
    },
    ...
    render() {
        ...
        const tabHTML = `
            <div class="flex gap-2 mb-6 bg-black/40 p-1 rounded-2xl border border-white/5">
                <button onclick="WithdrawSystem.switchTab('withdraw')" class="flex-1 py-3 rounded-xl flex items-center justify-center gap-2 transition-all ${this.currentTab === 'withdraw' ? 'bg-emerald-500 text-black shadow-lg' : 'text-gray-500 hover:text-white'}"><span class="text-[9px] font-black uppercase">Withdrawal</span></button>
                <button onclick="WithdrawSystem.switchTab('deposit')" class="flex-1 py-3 rounded-xl flex items-center justify-center gap-2 transition-all ${this.currentTab === 'deposit' ? 'bg-emerald-500 text-black shadow-lg' : 'text-gray-500 hover:text-white'}"><span class="text-[9px] font-black uppercase">Deposit</span></button>
            </div>`;
        ...
        if (this.currentTab === 'deposit') {
            contentHTML = `
                <div class="flex flex-col items-center justify-center h-64 text-center opacity-80 animate-in">
                    <div class="w-20 h-20 bg-yellow-500/10 rounded-full flex items-center justify-center mb-4 border border-yellow-500/20"><i class="fas fa-shield-alt text-3xl text-yellow-500 animate-pulse"></i></div>
                    <h3 class="text-lg font-black text-white uppercase mb-2">Security Update</h3>
                    <p class="text-[9px] text-gray-400 px-8 leading-relaxed">The Deposit Gateway is currently undergoing security maintenance. Please check back shortly.</p>
                </div>`;
        } else {
            ...
            let feeLabel = "", inputPlaceholder = "", inputReadOnly = "", inputValue = "", helperText = "";
            ...
            const coinGridHTML = Object.keys(this.rates).map(key => {
                ...
                let iconHtml = key === 'TON' ? `<span class="text-[9px] font-black uppercase text-blue-400 drop-shadow-md">💎 ${key}</span>` : `<span class="text-[9px] font-black uppercase text-gray-300 group-hover:text-emerald-400">${key}</span>`;
                return `<button id="btn-coin-${key}" onclick="WithdrawSystem.selectCoin('${key}')" class="${isActive ? activeClass : inactiveClass}">${iconHtml}</button>`;
            }).join('');
            ...
            contentHTML = `
                <div class="flex gap-2 mb-4 bg-black/40 p-1 rounded-2xl border border-white/5 animate-in">
                    <button onclick="WithdrawSystem.selectMethod('faucetpay')" class="flex-1 py-3 rounded-xl flex flex-col items-center gap-1 transition-all ${this.selectedMethod === 'faucetpay' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}"><span class="text-[9px] font-black uppercase">FaucetPay</span><span class="text-[7px] opacity-80">Instant</span></button>
                    <button onclick="WithdrawSystem.selectMethod('direct')" class="flex-1 py-3 rounded-xl flex flex-col items-center gap-1 transition-all ${this.selectedMethod === 'direct' ? 'bg-orange-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}"><span class="text-[9px] font-black uppercase">Direct Chain</span><span class="text-[7px] opacity-80">Fee ${feePercent}%</span></button>
                </div>
                <div class="flex-1 overflow-y-auto no-scrollbar pr-1 animate-in">
                    ...
                    <div id="wd-coin-grid" class="grid grid-cols-3 gap-2 mb-4">${coinGridHTML}</div>
                    <div class="flex flex-col gap-4 mb-4">
                        ...
                        <div class="space-y-2">
                            <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">${quickAmtHTML}</div>
                            <div>
                    ...
                    <div class="bg-black/20 rounded-xl p-3 border border-white/5 mb-4 space-y-1">
                        ...
                        <div class="flex justify-between items-center text-[8px] text-gray-400"><span>Estimated Receive (Min):</span><div class="text-right"><p id="wd-preview-amount" class="text-sm font-black text-white leading-none">0.00</p><p id="wd-preview-symbol" class="text-[7px] font-bold text-emerald-500 uppercase">${this.selectedCurrency}</p></div></div>
                    </div>
                </div>
